"""
Face_tracking01 - Fixed Camera Version
...
Fixed camera initialization to prevent freezing issues
"""
...
if sys.platform == "linux":
    try:
        ...
        print("picamera module not found, using standard camera")
        RASPBERRY_BOOL = False

ROBOT_IP = '192.168.10.152'
...
# Initialize camera with simplified settings
print("Initializing camera...")
if RASPBERRY_BOOL:
    # Raspberry Pi camera
    try:
        from imutils.video import VideoStream
        vs = VideoStream(usePiCamera=True, resolution=video_resolution, framerate=13).start()
    except Exception as e:
        print(f"Failed to initialize Pi camera: {e}")
        print("Falling back to USB camera")
        RASPBERRY_BOOL = False

if not RASPBERRY_BOOL:
    # Use standard OpenCV VideoCapture for USB/built-in cameras
...
def read_camera_frame():
    """
    Read a frame from the camera, handling both VideoStream and VideoCapture
    """
    if RASPBERRY_BOOL:
        frame = vs.read()
    else:
...
def move_to_face(list_of_facepos,robot_pos):
    ...
    oriented_xyz_coord = oriented_xyz.pose_vector.array
...
try:
    print("starting loop")
    print("Press 'q' to quit the face tracking")
    while True:
